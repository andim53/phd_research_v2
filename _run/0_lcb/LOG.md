...
## 2026-09-08 — commit INSTR #6 (novelty filter) + INSTR #7 (KDE bandwidth) code
- Owner applied INSTR #6 and INSTR #7 to the code (uncommitted); per owner, commit
  the code first, then update README.AI.md.
- run_analysis_indices.py -> 2.11.0: --novelty-dist filter (greedy raw-Euclidean
  Fingerprint dedup for Stages 2/3, Stage 1 raw) + --h (kde_bw) Gaussian-KDE
  bandwidth for Stages 2/3.
- plot_structure_landscape.py -> 1.2.0: kde_bw param threaded into the Stage-2
  density KDE (gaussian_kde(bw_method=kde_bw)).
- VERSIONS.md rows appended for 2.10.0 (novelty) and 2.11.0 + 1.2.0 (kde_bw).
- NOTE: regenerated stage JSONs under 11_bTa/*/analysis_indices are tracked in
  git but were regenerated by the INSTR #8 reproduce run; left out of this code
  commit (regenerable data). Decided separately whether to commit them.
- Committed code + VERSIONS.md + this LOG entry.

## 2026-09-08 — README.AI.md updated to current code state + mechanism overview
- Owner: update README.AI for the current state of the code, include all
  necessary context so a future agent (0 context) can immediately write new code;
  add a whole-mechanism overview. Owner chose: commit the uncommitted INSTR #6/#7
  code first, then update README.
- Committed INSTR #6 (--novelty-dist, runner 2.10.0) + INSTR #7 (--h/kde_bw,
  runner 2.11.0, plot_structure_landscape 1.2.0) code + VERSIONS rows + LOG.
- README.AI.md changes:
  - New §1b "Project mechanism — whole-pipeline overview" (data flow, one-runner-
    one-leaf, the two-env split, 3-stage pipeline, novelty filter + KDE bandwidth
    controls, JSON self-description system, XRD sub-pipeline, git discipline).
  - Fixed stale versions: runner 2.9.0 -> 2.11.0 (tree + §2b), plot_structure_landscape
    1.1.0 -> 1.2.0, xrd_simulate 1.1.0 -> 1.4.3 (tree line).
  - §2b CLI list now includes --novelty-dist and --h with semantics.
  - §5 known-gaps: documented that some analysis_indices/*.json are historically
    tracked (re-running modifies them; don't blanket-add).
