...
a_w = 3.189001 # Ta: 3.331167 # optim # 3.30
supercell = (3, 3, 3)
kpts = (1, 1, 1) 
ncores = 24 # 16 genkai
...
for seed in range(100):
    ...
    # Make W with Boron
    w_bulk = bulk("W", "bcc", a=a_w, cubic=True) * supercell
    w_bulk.set_cell(w_bulk.cell * scale_cell, scale_atoms=True)

    wb_bulk, n_b, n_ta = add_B_concentration(
        w_bulk, cB=cB, make_structure=True
    )

    print(f"{n_b=}")
    
    write("wb_bulk.xsf",wb_bulk)
    write("w_bulk.xsf", w_bulk)

    """
    Environment is an empty cells, size of ta_bulk. 
    """
    template = Atoms("", cell=wb_bulk.cell.copy(), pbc=True)
    environment = Environment(
        template=template,
        symbols=wb_bulk.get_chemical_formula(),
        confinement_cell=template.cell.copy(),
        confinement_corner=np.array([0, 0, 0]),
        box_constraint_pbc=[True, True, True],
    )

    n_rattle = len(wb_bulk)
    generators = [
