...
a_pt = 3.975534 # pt # 3.189001 # Ta: 3.331167 # optim # 3.30
supercell = (3, 3, 3)
kpts = (1, 1, 1) 
ncores = 24 # 16 genkai
cB = 0.00 # 0.05 # 0.01 # B concentrations (%)
N_iterations = 100 # AGOX iterations
# num_candidates = {0: [10, 0, 0], 10: [5, 5, 0], 25: [0, 5, 5]}
num_candidates = {0: [10, 0], 10: [5, 5], 25: [0, 10]}
sample_size = 10
...
for seed in range(100):
    ...
    # Make Pt with Boron
    pt_bulk = bulk("Pt", "fcc", a=a_pt, cubic=True) * supercell
    pt_bulk.set_cell(pt_bulk.cell * scale_cell, scale_atoms=True)

    ptb_bulk, n_b, n_ta = add_B_concentration(
        pt_bulk, cB=cB, make_structure=True
    )

    print(f"{n_b=}")
    
    write("ptb_bulk.xsf",ptb_bulk)
    write("pt_bulk.xsf", pt_bulk)

    """
    Environment is an empty cells, size of ta_bulk. 
    """
    template = Atoms("", cell=ptb_bulk.cell.copy(), pbc=True)
    environment = Environment(
        template=template,
        symbols=ptb_bulk.get_chemical_formula(),
        confinement_cell=template.cell.copy(),
        confinement_corner=np.array([0, 0, 0]),
        box_constraint_pbc=[True, True, True],
    )

    n_rattle = len(ptb_bulk)
    generators = [
    ...
    try:
            ...
            # permut_candidate = generators[2]( # 2
            #     sampler_test,
            #     environment
            # )[0]
