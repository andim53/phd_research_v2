#!/bin/sh
# PJM batch script for the Ni8/Au(4,4,2) EMT benchmark
# (regular LCB vs Novelty-LCB auto global-minimum window).
# Submit:  pjsub j_benchmark.sh |  monitor: pjstat  |  cancel: pjdel

##PJM -L rscgrp=a-pj24001864
#PJM -L rscgrp=a-batch
#PJM -L vnode-core=24
#PJM --mpi proc=24
#PJM -L elapse=48:00:00
#PJM -j
#PJM -X

source ~/.bashrc
conda activate gpaw_env
module load intel
module load impi

# NOTE: this benchmark uses EMT + AGOX (no GPAW). It assumes the active conda
# env has AGOX/ASE/EMT installed (the same env that runs the Fe/MgO AGOX search).
# gpaw_env is used here to match j_novel.sh; if it lacks AGOX, switch to agox_v2.

echo "Launching Ni8/Au(4,4,2) EMT benchmark (extended grid: 10 seeds x iter 100-500 x lambda 2-5 = 250 runs)"
echo "Date: $(date)"
OMP_NUM_THREADS=1 python ./main_benchmark.py

echo "Benchmark done: $(date)"
