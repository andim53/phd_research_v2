...
# Make the package importable when run as a script from any dir
_HERE = Path(__file__).resolve().parent
_PROJ = _HERE.parent
if str(_PROJ) not in os.sys.path:
    os.sys.path.insert(0, str(_PROJ))
...
def make_environment(template: Atoms,
                     symbols: str = DEFAULTS["symbols"]) -> Environment:
    return Environment(
        template=template,
        symbols=symbols,
...
def build_agox(
    ...
    env = make_environment(template, symbols=p["symbols"])
