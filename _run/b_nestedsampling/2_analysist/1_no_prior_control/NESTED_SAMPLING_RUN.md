...
## Canonical heavy run command
This is a **full-pipeline** heavy job (highest evidence resolution, longest budget,
analysis included). Replace the `python ./run_nested_sampling.py ...` line in
`job.sh` with:
...
- `--n-live 200` — very high evidence resolution (low variance in log Z) at the cost
  of more GPR evaluations per iteration.
- `--n-iters 2000` — long run; drives the prior-volume shrinkage deep toward the
...
- `OMP_NUM_THREADS=1` avoids OpenMP oversubscription inside the batch node.

## Tuning knobs (choose per what you want)

| Goal | Option | Direction |
|---|---|---|
| Finer evidence / better resolution | `--n-live` | higher (e.g. 100–500) |
| Deeper search toward minimum | `--n-iters` | higher (e.g. 1000–5000) |
...
clobber e.g. each other), each with `--n-live 100 --n-iters 1000 --temp <Ti>`:

```bash
for T in 100 200 300 500 1000; do
  OMP_NUM_THREADS=1 python ./run_nested_sampling.py \
      --temp $T --n-live 100 --n-iters 1000 --perturb 0.01 \
      --perturb-symbols Fe --output ./ns_T${T} --rng 42
done
```
Each produces its own evidence Z (→ free energy F = -k_B·T ln Z) and its own
analysis dir, so the temperature dependence of the partition function / free
energy can be read directly from the evidence across T.
> Run each T as a separate `pjsub` job or a PJM array to use the 64 cores /
> 120 h budget efficiently (the script itself is single-process by design after
> the `use_ray=False` fix, so run one job per node rather than MPI-parallelising
> the sampler).

## Supercomputer-specific notes
- The elapsed allowed 120 h is far more than a single heavy run needs to finish,
  but supports the T-scan and/or large `--n-live`/`--n-iters` options.
- Since the fix `use_ray=False` makes the sampler single-process, scale out by
  submitting **many independent jobs** (one per T and/or per `--rng` seed) rather
  than one `mpiexec` invocation — this is the efficient use of the node.
- Wall-clock estimate: GPR training ~2–5 min (single-core); sampling cost scales
  ~ linearly with `--n-iters ×` effective draws per iteration. 2000 iters / 200 live
  is comfortable within the budget.
