# TUTORIAL — Reproduce run b13_gpr_accuracy_boron3_cv50

Step-by-step guide to reproduce this run (GPR accuracy, 50-fold CV, uncertainty,
bin 0.1, Fe_z system, boron3 dataset) in isolation. Level: intermediate.

## Prerequisites

- Conda env **`agox_v2`** (AGOX 3.10.2 + ASE 3.25.0) for local runs:
  `/home/think/miniconda3/envs/agox_v2/bin/python`.
- HPC pjsub (PJM) for the batch run (`gpaw_env`, set in `j_b13_gpr_accuracy_boron3_cv50.sh`).
- Dataset present in this dir: `dataset/seed_*/1_db/db_*.db` (6 DBs, B3Fe25Mg25O25).

## Step 0 — What this run does

A single invocation of `gpr_accuracy.py` in **50-fold CV + uncertainty** mode at
**bin-width 0.1** on the **Fe_z (island-height) system** of the **boron3** Fe/MgO
dataset — identical parameters to `_runs/b3_gpr_accuracy_boron_cv50`, but on
B3Fe25Mg25O25 (597 structures):
...
50-fold CV trains 50 GPRs (one per fold); ~11 test structures per fold on this
smaller 568-structure (post-cut) set. Higher CV → robust held-out pooling but
...
outliers that otherwise break the GPR fit). See README.md.

## Step 1 — Local run (validate on a small scale first if desired)

```bash
cd /home/think/Desktop/research/_run/b_nestedsampling/_runs/b13_gpr_accuracy_boron3_cv50
...
- **Composition:** dataset must stay uniform B3Fe25Mg25O25 for the single descriptor.
