...
/home/think/miniconda3/envs/agox_v2/bin/python main.py \
    ...
    --output ./ns_output_tfree_walk_emax04_exclworst_noxsf_novelty_boron3_iter5000 --rng 42
```
Repeat the same command with `--n-iters 10000` / `20000` and the matching
`..._boron3_iter10000` / `..._boron3_iter20000` output dir.
...
- `--perturb-symbols Fe,B` — BOTH Fe and B atoms move during prior sampling / the walk;
  all others (Mg/O) fixed.
- `--e-max-per-atom 0.4` — drop DB structures above 0.4 eV/atom relative to the min (448 of 597
  remain for the boron3 dataset).
...
## Note on the boron3 dataset

The run reads `dataset/seed_*/1_db/db_*.db` (6 non-empty seeds, seed_6 empty/excluded).
`main.py` derives the descriptor and `n_atoms` from the data (78 atoms, B3Fe25Mg25O25), so the
multi-seed GPR works for this composition. It uses a single global Fingerprint — the boron3 set
is uniform (B3Fe25Mg25O25), which the descriptor requires.
