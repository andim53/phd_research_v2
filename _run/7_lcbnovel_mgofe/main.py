...
ncores = 64 #24 # 16 cores for genkai
supercell = (5, 5, 1)
kpts = (1, 1, 1)
kappa=2
N_iterations = 100

# --- Novelty-LCB energy window parameters ---
# These MUST be calibrated for Fe/MgO before trusting results.
# Run a short standard-LCB search first to see the energy range,
# then set target_energy to the band centre and delta_E to ~0.5-1.0 eV half-width.
...
for seed in range(3, 105):
    ...
    # Acquisitor — Novelty-LCB (replaces standard LCB)
    # Energy window: [target_energy - delta_E, target_energy + delta_E]
    # Candidates outside this window are excluded from selection.
    # NOTE: target_energy=0.0 is uncalibrated — calibrate before trusting results.
