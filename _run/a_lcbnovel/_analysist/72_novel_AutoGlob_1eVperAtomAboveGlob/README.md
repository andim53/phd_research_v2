...
### Where κ (kappa) is applied — and the order of operations

In the Novelty-LCB code, **κ appears in exactly one place**: the LCB *relaxation
surface* used to pre-relax candidates — **not** in the novelty selection score.

1. **Where κ is applied.** In `novelty_lcb/acquisitor.py`,
   `get_acquisition_calculator()` builds a `LowerConfidenceBoundCalculator` with

       lcb_acquisition_energy(E, sigma, kappa) = E − κ·σ
       lcb_acquisition_force(E, F, sigma, sigma_force, kappa) = F − κ·σ_force

   (module-level free functions, closed over the scalar `kappa`). This calculator is
   handed to `ParallelRelaxPostprocess` (in `main.py`'s `build_stack`), which
   pre-relaxes every candidate on the cheap surrogate surface `E − κ·σ` before the
   expensive GPAW evaluation. So κ shapes *where each candidate relaxes to* on the
   surrogate.

2. **Where κ is NOT applied.** `calculate_acquisition_function()` — the actual
   selection — uses `a(x) = σ(x) + λ·Novelty(x)` only (line `values[i] = -(sigma +
   self.novelty_weight * novelty)`). κ does **not** appear there. So κ does not directly
   decide which candidate is picked; it only sets the relaxation surface that candidates
   slide down before selection.

3. **Does it run regular LCB then Novelty-LCB? Which goes first?** It is **not** two
   separate sequential searches. It is **one integrated acquisitor**, and within each
   iteration the two parts happen in this order:
   - **First (relaxation):** candidates are pre-relaxed on the **LCB surface**
     `E − κ·σ` (this is the "LCB / regular-LCB" role of the acquisitor, driven by κ).
   - **Second (selection):** the relaxed candidates are scored with the **Novelty-LCB**
     acquisition `σ + λ·Novelty` (driven by λ) inside the energy window, and the best
     one is chosen for GPAW evaluation.

   So the "LCB part" (κ) runs **before** the "novelty part" (λ) on the same candidate
   pool — LCB first (relax), novelty second (select). It is a combined
   relax-then-select pipeline, not "finish a regular-LCB search, then start a separate
   novelty search." (A separate, plain `LowerConfidenceBoundAcquisitor` exists only as
   the *comparison* run in the benchmarks/analysis, not inside the Novelty-LCB code.)
