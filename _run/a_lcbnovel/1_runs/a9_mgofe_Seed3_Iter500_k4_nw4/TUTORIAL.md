# TUTORIAL — Reproduce run a9 (Novelty-LCB Fe/MgO, seed 3, 500 iter, kappa=4, nw=4)

Step-by-step guide to reproduce **this specific run** (`a9_mgofe_Seed3_Iter500_k4_nw4`)
...
- Acquisition: Novelty-LCB `a(x) = σ(x) + λ·Novelty(x)` with **λ = 4.0**.
...
- Treatment: **novelty_weight = 4.0** (vs 1.5 in a5, 2.0 in a7, 3.0 in a8).

## Step 1 — Verify the code compiles (local, no GPAW)

```bash
cd /home/think/Desktop/research/_run/a_lcbnovel/1_runs/a9_mgofe_Seed3_Iter500_k4_nw4
...
--kappa 4 --novelty-weight 4 --out-root ./output`. To change the run, edit `SEED=` /
...
3. **Outputs are regenerable** — `output/` and `*.db` are gitignored; only code + docs
   are tracked.
4. **This is the novelty_weight=4.0 leg of a sweep** — keep λ distinct from a7 (2.0)
   and a8 (3.0); change only what this run is meant to probe, so the sweep stays
   comparable.
