# TUTORIAL — Reproduce run a6 (Novelty-LCB Fe/MgO, seed 3, 500 iter, kappa=5)

Step-by-step guide to reproduce **this specific run** (`a6_mgofe_Seed3_Iter500_k5`)
...
- Treatment: **kappa = 5.0** (vs kappa 2.0 in a2, 3.0 in a4, 4.0 in a5). Kappa sets the
  LCB surrogate-relaxation surface `E − kappa·sigma`.

## Step 1 — Verify the code compiles (local, no GPAW)

```bash
cd /home/think/Desktop/research/_run/a_lcbnovel/1_runs/a6_mgofe_Seed3_Iter500_k5
...
--kappa 5 --out-root ./output`. To change the run, edit `SEED=` / `N_ITERATIONS=` /
...
3. **Outputs are regenerable** — `output/` and `*.db` are gitignored; only code + docs
   are tracked.
4. **This is the kappa=5 leg of a sweep** — keep kappa distinct from a4 (3) and a5 (4);
