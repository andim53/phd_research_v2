# TUTORIAL — Reproduce run a2 (Novelty-LCB Fe/MgO, seed 3, 500 iterations)

Step-by-step guide to reproduce **this specific run** (`a2_mgofe_Seed3_Iter500`) from
...
- Budget: **500 AGOX iterations** — the middle of the three sibling runs, the
  standard-length search (more complete than a1's 300, less than a3's 700).

## Step 1 — Verify the code compiles (local, no GPAW)

```bash
cd /home/think/Desktop/research/_run/a_lcbnovel/1_runs/a2_mgofe_Seed3_Iter500
...
4. **This run is the middle one (500)** — use a1 (300) for a quick check or a3 (700)
   for the deepest sampling; keep the runs comparable.
