# TUTORIAL — Reproduce run a4 (Novelty-LCB Fe/MgO, seed 3, 500 iter, kappa=3)

Step-by-step guide to reproduce **this specific run** (`a4_mgofe_Seed3_Iter500_k3`)
...
- Treatment: **kappa = 3.0** (vs kappa 2.0 in a2, 4.0 in a5, 5.0 in a6). Kappa sets the
  LCB surrogate-relaxation surface `E − kappa·sigma`.

## Step 1 — Verify the code compiles (local, no GPAW)

```bash
cd /home/think/Desktop/research/_run/a_lcbnovel/1_runs/a4_mgofe_Seed3_Iter500_k3
...
--kappa 3 --out-root ./output`. To change the run, edit `SEED=` / `N_ITERATIONS=` /
...
3. **Outputs are regenerable** — `output/` and `*.db` are gitignored; only code + docs
   are tracked.
4. **This is the kappa=3 leg of a sweep** — keep kappa distinct from a5 (4) and a6 (5);
