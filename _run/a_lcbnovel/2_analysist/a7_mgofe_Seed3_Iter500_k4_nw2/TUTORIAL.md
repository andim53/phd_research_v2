# TUTORIAL — Reproduce run a7 (Novelty-LCB Fe/MgO, seed 3, 500 iter, kappa=4, nw=2)

Step-by-step guide to reproduce **this specific run** (`a7_mgofe_Seed3_Iter500_k4_nw2`)
...
- Acquisition: Novelty-LCB `a(x) = σ(x) + λ·Novelty(x)` with **λ = 2.0**.
...
- Treatment: **novelty_weight = 2.0** (vs 1.5 in a5, 3.0 in a8, 4.0 in a9).

## Step 1 — Verify the code compiles (local, no GPAW)

```bash
cd /home/think/Desktop/research/_run/a_lcbnovel/1_runs/a7_mgofe_Seed3_Iter500_k4_nw2
...
--kappa 4 --novelty-weight 2 --out-root ./output`. To change the run, edit `SEED=` /
...
3. **Outputs are regenerable** — `output/` and `*.db` are gitignored; only code + docs
   are tracked.
4. **This is the novelty_weight=2.0 leg of a sweep** — keep λ distinct from a8 (3.0)
