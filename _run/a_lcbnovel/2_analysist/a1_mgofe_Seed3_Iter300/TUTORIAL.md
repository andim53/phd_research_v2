...
```bash
cd /home/think/Desktop/research/_run/a_lcbnovel/1_runs/a1_mgofe_Seed3_Iter300
...
`2_analysist/run_analysis_a_runs.py` (same 3-stage pipeline as the multi-seed runner:
best-so-far progression → PCA landscape → Boltzmann probability). Run it from the
`2_analysist/` directory with the `agox_v2` env:
