...
- Reference example for the B-doping approach:
  `/home/think/Desktop/research/_archive/_analysist/1_result/66_MgOFe_20B`.

## What to expect

- Physics: **B7Fe25** deposition on a fixed MgO(001) substrate (Mg25O25 + B7Fe25 =
  82 atoms). B atoms are placed in Fe-hollows via `add_adsorbate_to_hollows`.
...
- B count: **7** = number of real square hollows found in the 25-Fe layer by the fixed
  `add_adsorbate_to_hollows` v1.1.0 (proper hollow-site detection).

## Step 1 — Verify the code compiles (local, no GPAW)

```bash
cd /home/think/Desktop/research/_run/a_lcbnovel/_runs/a10_mgofeb_Seed3_Iter500
...
"from scripts.build_fe_stack import build_fe_stack; from scripts.add_adsorbate_to_hollows import add_adsorbate_to_hollows; from ase.build import surface, bulk; slab=build_fe_stack(surface(bulk('Fe','bcc',a=2.870190,cubic=True),(0,0,1),layers=1,vacuum=20),num_layers=1,vacuum=20).repeat((5,5,1)); d=add_adsorbate_to_hollows(slab,symbol='B',height=0,num_atoms=7,seed=3); print(d.get_chemical_formula(), len(d))"
```

Expected: `B7Fe25`, 32 atoms (the mobile layer). The full cell is 82 atoms with the
...
grep "Doped deposition layer" output_seed_3.txt   # should show B7Fe25
```
...
2. **B count reflects real hollows** — `add_adsorbate_to_hollows` v1.1.0 detects the
   real square hollows in the Fe layer (a 5×5 Fe grid has 16 hollows). This run sets
   `NUM_ATOMS_ADD=7` in `main.py`, so the mobile layer is **B7Fe25**. (The old buggy
   grouping produced B6Fe25 and could place a B exactly on top of an Fe atom, causing
   the `AtomsTooClose` crash — fixed.)
...
5. **Composition is uniform by construction** — B is added to the initial deposition
   slab, so all candidates share B7Fe25 and the single global Fingerprint stays valid.
6. **Keep `.py` files in their folders** — `main.py` lives at the run root; the builder
   scripts live in `scripts/` and the package in `novelty_lcb/`. Do **not** copy `.py`
   files flat into the run root (a `cp ... scripts/*.py <run>/` sync scatters them).
   When re-syncing from the project root, copy into the matching subdirs:
   `cp main.py <run>/ && cp scripts/*.py <run>/scripts/ && cp novelty_lcb/*.py <run>/novelty_lcb/`.
   Also note `_runs/a10/main.py` is **run-specific** (`NUM_ATOMS_ADD=7`) and must not be
   overwritten by the root no-B default (`NUM_ATOMS_ADD=0`).
