"""
Fe-Co-B/MgO — one monolayer of FeCo on MgO(001) with boron inserted in the metal layer.

New run (`data/cofe_v2/fecobmgo`).  Same purpose as its boron-free counterpart
(`data/cofe_v2/fecomgo`): rebuild the archived Fe-Co host as a controlled counterpart of the Fe
host, per `_archive/cofe/README.md` -> "Reviving the host".

Provenance
  Copied from `data/_archive/fecobmgo/main.py`, which produced the archived MT-4 value.  Everything
  is unchanged except the five items below.

CHANGED vs the archived script
  1. `interpolation_factor` 1 -> 0, so the cell is built on the film lattice (see the longer note in
     `../fecomgo/main.py`).  The archived run sat on a_MgO/sqrt(2) = 2.97833 A.
  ...
  2. `a_feco` 2.839177 -> 2.870190 A, so the Co host shares the Fe host's cell instead of sitting
     1.09 % away from it.
  3. `num_atoms_add` 2 -> 3, matching the three boron atoms inserted by the in-scope Fe-B model
     (`data/febmgo/main.py`).  The archived run inserted only two, so the two B models differed in
     boron content as well as in host.
  4. A third generator, `PermutationGenerator`, with the three-generator schedule
     {0: [20, 0, 0], 10: [10, 5, 5], 25: [0, 10, 10]}.  The archived FeCoB run used only two
     generators while the archived FeCo run used three; both new Co runs now use three so that the
     boron effect inside the Co host is not confounded with the generator set.
  5. `latt_log.md` now records the cell actually built (`a_custom`) and its strain.

UNCHANGED, deliberately
  - B placed at hollow sites of the Fe/Co surface (`target_symbols=("Fe", "Co")`), height 0
    (in-plane insertion), B choosing its sites with `seed=seed` -> the B positions DO vary per
    replicate, unlike the Co decoration (see below)
  - GPAW LCAO / dzp / PBE, mixer weight 50, convergence energy 1e-3, occupations width 0.10
    (the archived Co values; the Fe host uses 100 / 1e-4 / 0.05)
  - ncores = 16, matching this run's `vnode-core=16` allocation in `job.sh`
  - kappa = 2, N_iterations = 100, n_co = 7, supercell (5, 5, 1), kpts (1, 1, 1)
...
CHANGED (was 1): 0 puts the cell on the film, as in the Fe host.  Simul: 0, 0.25, 0.5, 0.75, 1
"""
...
ncores = 64 #24; 16 cosres for genkai  (this run's job.sh asks for vnode-core=16)
supercell = (5 , 5, 1)
kpts = (1, 1, 1)
...
CHANGED (was 2): 3 boron atoms, matching the in-scope Fe-B model (`data/febmgo/main.py`).
"""
...
"""
Co decoration of the 1 ML film.  `feco_randamize_generater` defaults to seed=0, so the Fe/Co pattern
is FIXED and identical in every seed AND identical to the pattern in `../fecomgo` — the same 7 of
25 sites become Co in both models.  That is what makes the with/without-boron pair comparable.
Pass `seed=seed` below to vary it per replicate instead.
"""
...
for seed in range(103):
	...
	# Boron controls
	slab_deposition = add_adsorbate_to_hollows(slab_deposition, symbol=symbol_add,height=z_height_add, num_atoms=num_atoms_add,seed=seed, target_symbols=("Fe","Co"))
	write(f'{path_xsf}/slab_deposition_candidate.xsf', slab_deposition)
