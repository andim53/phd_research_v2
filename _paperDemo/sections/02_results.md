...
<!-- DRAFT v2 · section 02 of the manuscript (markdown-first, pre-LaTeX)
     ...
     Grounded in CLAIMS.md v7. MT-6 is WITHDRAWN and no flat-fraction comparison appears here;
...
random seeds (13 / 6 / 5 / 4 for Fe / Fe-B / Fe-Co / Fe-Co-B), and structures from one search
are correlated with each other, so the independent unit is the search, not the structure. Taking each search's own flat-basin minimum
...
corresponding median shifts are 0.035 and 0.032 eV/atom — i.e. the effects are not carried by a
single favourable search. The Fe-host effect and the cobalt null are therefore robust to the
choice of search, whereas the Fe-Co-host effect, although of the same size, **is not yet
resolved by the four searches available for Fe-Co-B** and is reported here as a trend; further
searches of that model are the natural way to settle it.
