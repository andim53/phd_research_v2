...
20~\AA{} of vacuum and periodic boundaries in-plane. \textbf{The in-plane lattice of the simulation
...
\textbf{MgO is compressed in-plane by 3.6\,\%}, i.e.\ the mismatch is accommodated by the substrate,
not by the film---the Fe film remains at its own equilibrium lattice constant and is not strained
in-plane. The substrate is then \textbf{held fixed in that compressed state} and only the film is
allowed to relax.

\subsection{Biased global optimisation (AGOX / GOFEE)}
\label{sec:bias}

Structures were sampled with AGOX \cite{agox2020}, using a \textbf{GOFEE}-type surrogate-driven
...
(LCB) acquisition function ($\kappa = 2$). Each search ran 100 iterations.

\textbf{Three-phase biased exploration.} Candidate generation is not uniform over the run but
proceeds in three phases selected by the iteration counter $i$, mixing generators of different
perturbation scale. Both models combine a \textbf{small-scale} heterostructure-aware randomiser
(rattle amplitude 1.5, all film atoms) with a \textbf{large-scale} rattle generator (half the film
...
\textbf{Bias.} The exploration is deliberately \textbf{biased}: the search is seeded from a
\textbf{flat} metal layer---the reference film geometry, a single-monolayer-thick film rather than a
...
\textbf{Only completed searches are used.} Every number in this paper comes from searches that ran
the full 100-iteration budget: \textbf{13 completed searches for Fe/MgO and 6 for Fe-B/MgO}, 19 in
...
exception throughout the analysis, and it is decided from the \textbf{iteration count recorded for
...
evaluated with the DFT calculator below. \textbf{Only structures from iteration $\ge 10$ were used in
the analysis}, i.e.\ after relaxation had begun; earlier pre-relaxation placements were discarded.

\subsection{DFT settings}
\label{sec:dft}

Energies and forces were obtained with GPAW \cite{gpaw2014} in LCAO mode (double-zeta polarised
basis), the PBE exchange--correlation functional \cite{pbe1996}, a $(1\times1\times1)$ k-point mesh,
...
$\Gamma$-point sampling): the study targets \textbf{trends across compositions}, not converged
absolute energies (see Sec.~\ref{sec:scope}).

The interfacial Fe--O separation is a \textbf{construction parameter rather than a computed
...
\textbf{Film flatness $\dZ$}---the vertical span of the metal film,
$\dZ = z(\text{metal})_\text{max} - z(\text{metal})_\text{min}$, taken over the film's
\textbf{metal} atoms (Fe; boron is a metalloid and is not part of the flatness metric).
...
\textbf{Relative energy $dE/N$}---the per-atom energy above the system's global minimum,
...
For each model, the \textbf{flat-basin ground state} (lowest $dE/N$ among $\dZ \le 1.0$~\AA{}
...
The structures retained here are \textbf{not DFT-converged minima}: candidates were relaxed with the
...
experimentally reported structure of ultrathin Fe on MgO(001) is body-centred \textbf{tetragonal}
...
One further property of the model bounds its interpretation: the \textbf{strain convention is the
inverse of the experimental stack}. The simulation cell takes the Fe lattice constant and the
\textbf{substrate} is compressed to match it, so the Fe film is unstrained in-plane, whereas in a
