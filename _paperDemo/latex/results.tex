...
designed the search to return a \textbf{map of this two-phase landscape}: a GOFEE global optimisation
(Sec.~\ref{sec:bias}) \textbf{seeded from a flat reference layer}, so that the flat basin is
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{pes_2_systems.png}
...
\textbf{The island is the ground state in both models, despite the flat bias.} The lowest-energy
...
initial structures were built---it is genuinely higher in energy than the island. (MT-1)

\textbf{The flat configuration is a distinct, higher-energy basin.} Taking for each model the
...
landscape, not merely the high-$\dZ$ tail of a single minimum. (MT-2)

\textbf{The branches are families, not single structures.} The $\dZ$ values are continuous rather
...
fingerprint shows that each branch's low-energy set splits into \textbf{two} distinct structural
...
compresses the pair distances and the motif count is a \textbf{lower bound} on the structural
...
\textbf{Experimental correspondence.} The films in this study are about \textbf{one monolayer thick
...
three-dimensional: He-atom scattering shows \textbf{3D metal island growth} of Fe on MgO(001), with
...
scanning tunnelling microscopy finds that \textbf{sub-nanometre Fe grows three-dimensionally} on MgO,
...
scattering on five monolayers evaporated at room temperature likewise identifies \textbf{Volmer--Weber
...
two basins therefore map onto the two outcomes that are experimentally accessible \textbf{at this
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{flat_state_summary.png}
...
\textbf{Boron lowers the flat-state energy.} Adding boron lowers the flat basin's best member by
0.040~\eVA---from 0.1888 to 0.1493~\eVA---a 21\,\% reduction of the flat--island separation. (MT-3)
...
\textbf{Boron does not bond to the MgO interface.} Restricting to the low-energy window
...
interfacial bonding. (MT-7)

\textbf{Uncertainty from the choice of search.} Each model was searched from several independent
...
0.1882~\eVA{} with it on average (medians 0.2185 and 0.1834), a \textbf{median} shift of
...
permutation test over the 19 searches gives \textbf{$p = 0.0444$}, significant at the 5\,\% level. In
...
which searches happened to contain boron. The test is an \textbf{exact enumeration of all 27\,132
...
is the \textbf{separation between them}---the flat-basin minimum of Table~\ref{tab:flatbasin}---and
...
\begin{itemize}
\item The biased GOFEE search, seeded from a flat reference, maps both the flat (wetting) and island
  (dewetting) phases in both models, over $\dZ = 0.001$--$5.57$~\AA.
\item \textbf{The island is the ground state in both models} ($\dZ = 3.65$ and 3.77~\AA), even though
  the search was biased toward the flat configuration---the flat bias did not manufacture the result.
\item \textbf{The flat configuration is a distinct, higher-energy basin}, 0.1888~\eVA{} above the
  island for Fe/MgO and 0.1493~\eVA{} for Fe-B/MgO---in both cases a separate basin, not the
  high-$\dZ$ tail of a single minimum.
\item \textbf{Boron lowers the flat--island separation by 0.040~\eVA} (13 vs 6 completed searches,
  exact two-sided permutation $p = 0.0444$).
\item \textbf{Boron does not bond to the MgO interface}---it stays within the film, so its effect on
  the flat phase is film-internal.
\item The low-energy structures of each branch form a \textbf{small set of recurring motifs} rather
  than one repeated structure, and $\dZ$ is continuous---the two phases are families, and the
  energetics above are compared unweighted, since the sampled densities are biased and not physical
  (MT-8).
\end{itemize}
