#!/usr/bin/env python3
"""
Validate that a custom AGOX acquisitor's acquisition calculator pickles cleanly
across the Ray pool, BEFORE launching an expensive GPAW/DFT search.

Walks the exact crash path that kills custom-acquisitor runs at setup:
    NoveltyLCBAcquisitor.get_acquisition_calculator()
        -> LowerConfidenceBoundCalculator
        -> ParallelRelaxPostprocess(model=calc, ...)
        -> AGOX.run()

This detects the classic `cannot pickle 'sqlite3.Connection'` failure (caused by
passing bound methods into the acquisition calculator, which captures the acquisitor
and its sqlite-backed Database) without any heavy compute (EMT calculator, tiny system).

Run:
    /home/think/miniconda3/envs/agox_v2/bin/python scripts/validate_acquisitor_serialization.py
...
_PROJ = os.path.dirname(_HERE)          # project root (one level up from scripts/)
...
# Point this at YOUR custom acquisitor package (imported from the project root).
from novelty_lcb import NoveltyLCBAcquisitor


def build_minimal_stack(seed=7, out_dir="_smoke_out"):
    """Tiny Au-on-Au fcc100 EMT system -- mirrors production wiring but cheap."""
    ...
    acquisitor = NoveltyLCBAcquisitor(
        ...
        target_energy=0.0, delta_E=2.0, novelty_weight=1.5, kappa=2.0, order=3,
    )
...
def main():
    print("=" * 70)
    print("Serialization smoke test: custom acquisitor acquisition calculator")
    print("=" * 70)
    ...
    inspect = getattr(ray.util, "inspect_serializability", None)
    ...
    print("\nRESULT: PASS -- acquisitor serialization is fixed at code level.")
    return 0
