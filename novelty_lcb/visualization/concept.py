...
def gpr_mean(xp):
    ...
    kernel_sum = np.sum(
        np.exp(-0.5 * ((xp[:, None] - db_positions) / 1.2) ** 2), axis=1
    )
...
ax1 = axes[0]
ax1.plot(
    x, true_e, "k--", lw=1.2, alpha=0.6,
    label="True PES (unknown to GPR)"
)
ax1.plot(x, mu, "b-", lw=2.0, label="GPR mean mu(x)")
ax1.fill_between(x, mu - sigma, mu + sigma, color="orange", alpha=0.20,
                 label="GPR +/-sigma(x)")
ax1.plot(x, mu - sigma, "orange", lw=0.8, alpha=0.5)
ax1.plot(x, mu + sigma, "orange", lw=0.8, alpha=0.5)

ax1.axhspan(window_lo, window_hi, color="green", alpha=0.12,
             label="Energy window")
...
ax1.text(
    x[-1] - 0.3, E_target, f"E_target = {E_target:.1f} eV",
    color="green", fontsize=9, va="center", ha="right"
)

ax1.scatter(
    db_positions, db_energies, color="red", s=60, zorder=5,
    edgecolors="black", linewidths=0.8, label="DB structures"
)
for xi, ei in zip(db_positions, db_energies):
    ax1.annotate(
        f"  DB@{xi:.1f}", (xi, ei), fontsize=7, color="red",
...
if best_x is not None:
    ...
    ax1.scatter(
        [best_x], [best_mu], color="green", s=120, zorder=6,
...
ax1.set_title(
    "Panel 1 - Energy Landscape, GPR Prediction & Energy Window",
...
# --- Panel 2: Novelty ---
ax2 = axes[1]
ax2.plot(x, nov, color="purple", lw=2.0,
         label="Novelty(x) = min_i d_fp(x, x_i)")
...
ax2.set_title(
    "Panel 2 - Structural Novelty "
    "(distance to nearest DB structure)",
    fontsize=12, fontweight="bold",
)
...
ax3.plot(
    plot_x, plot_a, "g-", lw=2.2,
    label=f"a(x) = sigma(x) + {lam}*Novelty(x)  [in window]"
)
...
if np.any(excluded):
    ...
    ax3.scatter(
        x[excluded], np.full(np.sum(excluded), -0.02),
        color="red", s=3, alpha=0.5, marker="|"
    )

ax3.text(
    0.02, 0.95, f"lambda = {lam}", transform=ax3.transAxes,
    fontsize=10, va="top",
...
if best_x is not None:
    ax3.scatter(
        [best_x], [best_a], color="green", s=150, zorder=6,
        marker="*", edgecolors="black", linewidths=0.8
    )
    ax3.annotate(
        f"  max a(x) = {best_a:.3f}\n  at x = {best_x:.2f}",
...
ax3.set_title(
    ...
    "(out-of-window candidates excluded -> a(x) = -inf)",
    fontsize=12, fontweight="bold",
)
...
outpath = os.path.join(outdir, "novelty_lcb_concept.png")
