...
\end{center}
\caption 
{ \label{Fig_mgo} Reverse deposition test for an $\text{MgO}$ monolayer on an $\text{Fe(001)}$ substrate. (a) The continuous state density distribution $g(E)$ and (b) the structural landscape mapped using the first principal component ($\psi_{1d}$) from the eigenvalue composition of the Valle-Oganov descriptor \cite{valle2010}. (c) The temperature-dependent Boltzmann probability distribution $P(E)$ for the reverse-deposition ensemble, indicating that the flat ground state remains the dominant configuration even at elevated temperatures.} 
\end{figure} 
