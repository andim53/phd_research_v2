...
\end{center}
\caption 
{ \label{Fig_mgo} Reverse deposition test for an $\text{MgO}$ monolayer on an $\text{Fe(001)}$ substrate. (a) The continuous state density distribution $g(E)$ and (b) the structural landscape mapped using the first principal component ($\psi_{1d}$) from the eigenvalue composition of the Valle-Oganov descriptor \cite{valle2010}. (c) The identified ground-state configuration, shown via side view and top view, where the $\text{MgO}$ monolayer (orange and red spheres) remains completely flat on top of the underlying $\text{Fe(001)}$ substrate (green spheres).} 
\end{figure} 
